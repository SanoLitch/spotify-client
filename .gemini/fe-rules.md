...
- **Domain / Model Layer (Ядро):** Самый внутренний слой. Содержит состояние
  ...
  библиотек).
- **Application Layer:** Оркестрирует доменный слой для выполнения конкретных сценариев использования (use cases). Например, `updateOrderFieldUseCase`. Зависит только от доменного слоя.
- **Infrastructure / UI Layer:** Внешний слой. Содержит всё, что взаимодействует с внешним миром: React-компоненты, хуки, адаптеры для API (adapters), порты (ports/interfaces), контроллеры для UI-эффектов (нотификации, модалки). Зависит от Application и Domain слоев.
- **Styling:** Используйте Vanilla Extract (`.css.ts` файлы) для стилизации. Избегайте inline-стилей и runtime CSS-in-JS.
- **UI Primitives:** Используйте Radix UI для сложных интерактивных компонентов (диалоги, выпадающие списки и т.д.).
- **Theme:** Определяйте глобальные токены и темы через `createGlobalTheme` или `createTheme` в Vanilla Extract.
- **Components:** Предпочитайте создание собственных переиспользуемых UI-компонентов на базе Radix primitives.
- **Shared DDD:** Используйте `@libs/ddd` для общих Value Objects и базовых классов.
- **Exports:** Используйте ТОЛЬКО именованные экспорты (никаких `export default`).
- **Typing:** Явно типизируйте все DTO и объекты передачи данных. Избегайте `any` в тестах.
